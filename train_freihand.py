...
transform = ComposeKeyPoints(
    [To3ChannelsGrayscaleKeyPoints(), ResizeKeypoints(224), RandomMirrorKeyPoints(), RandomAffineKeyPoints((-60, 60)),
     CropToKeyPoints(3), ResizeKeypoints(224), ToTensorKeyPoints(), NormalizeKeyPoints((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
     ])

train_set = FreiHandDataset(
    "/disk1/ofirbartal/Projects/Dataset/FreiHAND",percent=1,
    transform=transform)

train_loader = torch.utils.data.DataLoader(train_set, batch_size=16, shuffle=True)

val_set = FreiHandDataset(
    "/disk1/ofirbartal/Projects/Dataset/FreiHAND",set_name='evaluation',percent=1,
    transform=transform)

val_loader = torch.utils.data.DataLoader(val_set, batch_size=16, shuffle=True)

checkpoints_path = "/disk1/ofirbartal/Projects/LandmarksExtraction/Checkpoints/FCN_checkpoints/freihand_gray_{}/".format(
...
writer = SummaryWriter(comment='_freihand_grayscale_')
torch.cuda.empty_cache()

# # model and visualization
main_gpu = 2
model = CoordRegressionNetwork(n_locations=21).cuda(main_gpu)
model = torch.nn.DataParallel(model, [2,3], 2)

# Visualize keypoints
# example for visualization
images, labels = next(iter(train_loader))

# writer.add_graph(model, images.cuda(main_gpu))
